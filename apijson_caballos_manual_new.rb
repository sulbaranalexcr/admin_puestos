...
def execute_insert
  date = Time.now
  puts "Insertando dia: #{date}"
  uri = URI("https://www.goalserve.com/getfeed/0a0d67dd3da64f48eb9b08d9a3e907ab/racing/usa?json=1&date=#{date.strftime('%d.%m.%Y')}")
  res = Net::HTTP.get(uri)
  datos = JSON.parse(res)
  datos['scores']['tournament'].each do |hip|
    next unless hip['date'] == date.strftime('%d.%m.%Y')
...
def execute_insert_australia
  date = Time.now
  uri = URI("https://www.goalserve.com/getfeed/0a0d67dd3da64f48eb9b08d9a3e907ab/racing/australia?json=1&date=#{date.strftime('%d.%m.%Y')}")
  res = Net::HTTP.get(uri)
  datos = JSON.parse(res)
  datos['scores']['tournament'].each do |hip|
    next unless hip['date'] == date.strftime('%d.%m.%Y')

    jornada = create_hip(hip['id'], hip['name'], hip['race'].count, date, 'AUSTRALIA')
    hip['race'].each do |carr|
      next unless carr.instance_of?(Hash)
...
execute_insert
puts 'Creando carreras para AUSTRALIA'
execute_insert_australia