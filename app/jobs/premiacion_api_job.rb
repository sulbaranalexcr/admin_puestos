class PremiacionApiJob
  ...
  def perform(data_premio,hipodromo_id, carrera_id,tipo)
    ...
    ids_integrador.each do |idi|
      ...
      data_premio.each do |prop|
        if usuarios_ids.include?(prop['id']) && ids_taq.include?(prop['taq_id'].to_i)
          user_array << { 'id' => prop['id'], 'transaction_id' => prop['transaction_id'], 'reference_id' => prop['reference_id'], 
                          'amount' => prop['amount'].to_f, 'details' => '', 'pay_amount' => prop['pay_amount'].to_f, 
                          'loser' => prop['loser'] }
        end
      end
      ...
      EnviosMasivo.update_or_create(carrera_id, idi, tipo.to_i, objeto_completo) 
  ...
  def generate_losers(carrera_id, ids_taq, all_users)
    sleep 10
    ...
    ids_losers.each do |los|
      ...
      all_losers << { 'id' => id_temp_loser, 'transaction_id' => tra_id, 'reference_id' => ref_id, 'amount' => 0, 'details' => '',
