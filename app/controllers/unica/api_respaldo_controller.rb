module Unica
  class ApiRespaldoController < ApplicationController
    ...
    def cierre_carrera_interno
      begin
        ...
        hipodromo = Hipodromo.find_by(abreviatura: hipodromo_id, activo: true)
    ...
    def close_race
      begin
        ...
        hipodromo = Hipodromo.find_by(abreviatura: hipodromo_id, activo: true, cierre_api: true)
        unless hipodromo.present?
          render json: { "code" => -1, "msg" => "Hipodromo no existe." }, status: 400 and return
        end
        carr = Carrera.find_by(hipodromo_id: hipodromo.id, id_api: "#{Time.now.strftime('%Y%m%d')}-#{hipodromo.abreviatura}-#{numero_carrera}")
