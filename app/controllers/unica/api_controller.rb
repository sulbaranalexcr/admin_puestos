module Unica
  class ApiController < ApplicationController
    ...
    #             "https://admin.betsolutionsgroup.com/unica/",
    #             "https://admin.tablasdinamica.com/unica/",
    #             "https://admin.rojosynegros.com/unica/",
    #             "https://admin.betsolutiongroup.com/unica/"]
    SISTEMAS = ["https://admin.betsolutiongroup.com/unica/", 
                "https://admin.betsolutionsgroup.com/unica/",
                "https://admintablas.gamehorses.com/unica/",
                "https://admin.tablasdinamica.com/unica/",
                "https://adminrojonegro.gamehorses.com/unica/", 
                "https://admin.rojosynegros.com/unica/"]
    ...
    def cierre_carrera_interno
      begin
        ...
        carr = Carrera.find_by(hipodromo_id: hipodromo.id, id_api: "#{Time.now.strftime('%Y%m%d')}-#{hipodromo.abreviatura}-#{numero_carrera}")
    ...
    def close_race
      begin
        ...
        hipodromo = Hipodromo.where(id: ids_jor).find_by(id_goal: hipodromo_id, cierre_api: true)

        unless hipodromo.present?
          # render json: { "code" => -1, "msg" => "Hipodromo no existe." }, status: 400 and return
          render json: { status: 'OK', message: 'Data de cierre de carrera recibida.' } and return
        end
        carr = Carrera.find_by(hipodromo_id: hipodromo.id, id_api: "#{Time.now.strftime('%Y%m%d')}-#{hipodromo.abreviatura}-#{numero_carrera}")
    ...
    def invalidate_horse
      ...
      params[:_json].each do |param|
        hipodromo_id = param[:racecourse_track_id]
        numero_carrera = param[:race_number].to_i
        numero_caballo = param[:horse_number]
        ids_jor = Jornada.where(fecha: Time.now.all_day).pluck(:hipodromo_id)
        hipodromo = Hipodromo.where(id: ids_jor).find_by(id_goal: hipodromo_id)
        next unless hipodromo.present?

        bus_jornada = hipodromo.jornada.where(fecha: Time.now.all_day)
        next unless bus_jornada.present?

        bus_carrera = Carrera.find_by(id_api: "#{Time.now.strftime('%Y%m%d')}-#{hipodromo.abreviatura}-#{numero_carrera}")
        next unless bus_carrera.present?
  
        buscar_caballo = bus_carrera.caballos_carrera.find_by(numero_puesto: numero_caballo)
        next unless buscar_caballo.present?

        buscar = CaballosRetiradosConfirmacion.where(hipodromo_id: hipodromo.id, carrera_id: bus_carrera.id, caballos_carrera_id: buscar_caballo.id)
        unless buscar.present?
          CaballosRetiradosConfirmacion.create(hipodromo_id: hipodromo.id, carrera_id: bus_carrera.id, caballos_carrera_id: buscar_caballo.id, status: 1 )
        end
      ...
      # retirar_pendiente(hipodromo.id,bus_carrera.id,buscar_caballo.id)
      # SISTEMAS.each do |sis_url|
      #   Thread.new { 
      ...
      #   }
      # end
      render json: { status: 'OK', message: 'Data de retirar caballo recibida.' }
    end
    ...
    def activate_horse
      ...
      #   render json: { "code" => -1, "msg" => "Caballo con el numero #{numero_caballo} no existe." }, status: 400 and return
      # end
    ...
    def post_time
      ...
      # Postime.create(user_id: 1, hora_anterior: bus_carrera.hora_carrera, nueva_hora: hora_nueva.strftime("%H:%M:%S"), carrera_id: bus_carrera.id)
    ...
    def premiar_interno
      ...
      #             "https://admin.betsolutiongroup.com/unica/premiacion_puestos/premiar_puestos",
