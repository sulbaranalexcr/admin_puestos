...
def exctract_from_admin(url)
  # uri = URI.parse('http://localhost:4000/proxy_royal')
  uri = URI.parse('https://admin.betsolutionsgroup.com/proxy_royal')
...
def extract_racecourses
  res = exctract_from_admin('https://apuestasroyal.com/taquilla/api.php?Key=puestosAR202422&Tipo=Jornada')
  res['response']['data']['Jornada']
end

def extract_runners(race_id)
  res = exctract_from_admin("https://apuestasroyal.com/taquilla/api.php?Key=puestosAR202422&Tipo=Corredores&Detalle=#{race_id}")
  res['response']['data']['Corredores']
end
...
def insert_hip(id, api_name, races, date, country, video)
  country = case country.downcase
    ...
    hip = Hipodromo.create(nombre: api_name, tipo: 2, nombre_largo: api_name, cantidad_puestos: country[1], abreviatura: '',
                         activo: false, pais: country[0], bandera: "#{country[0].downcase}.png", id_goal: id, id_video: video)
...
def execute_insert(racecourse)
  datos = exctract_from_admin("https://apuestasroyal.com/taquilla/api.php?Key=puestosAR202422&Tipo=Carreras&Detalle=#{racecourse['cardId']}")
  date = DateTime.strptime(racecourse['cardDate'], '%Y-%m-%d').to_time.beginning_of_day
  return unless %w[GB AU].include?(racecourse['countryCode'])
